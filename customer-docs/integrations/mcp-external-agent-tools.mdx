---
title: "MCP/external agent tools"
description: "Connect and maintain MCP/external agent tools with the right account, permissions, reconnect path, and failure checks."
keywords: ["MCP/external agent tools","Integrations","Ergo docs"]
---


![Annotated integrations page showing connected sources, reconnect states, and setup tabs.](../images/integrations-settings-annotated.svg)

## Who is this for?

- For primary admins, secondary admins, RevOps, and operators managing workspace integrations.
- Only appears when enabled for your workspace.
- Requires External agent tools.
- Ask your Ergo team if the feature does not appear.

## Before you start

- Sign in to the correct Ergo workspace.
- Have access to the External agent tools account or admin console you plan to connect.
- Use the account your team expects Ergo to read from or write through.
- If you are reconnecting, use the same account when possible and approve every requested scope.

## Setup steps

- Confirm your workspace has access to external agent tooling.
- Use the provided setup path for the agent or client you are connecting.
- Verify that the connected tool can see the intended Ergo context.
- Contact support for beta access, setup, or troubleshooting.

## Common issues

- The integration grant expired or was revoked.
- The reconnect was completed with a different account than expected.
- Required scopes were not approved.
- The connected service changed permissions or channel/calendar access.

## Related articles

- [Integrations](./index)
- [Troubleshooting](../troubleshooting/index)
- [Getting support](../start-here/getting-support)
